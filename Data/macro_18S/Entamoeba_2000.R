...
macro18S_COVERAGE_RAREF_2000 <- phyloseq_coverage_raref(physeq=macro18S_final_for_CB, coverage = 0.9, iter = 2000, replace = F, correct_singletons = F, drop_lowcoverage = T)

saveRDS(macro18S_COVERAGE_RAREF_2000, "/Users/parfreylab/Desktop/lab_member_files/bia/Calvert_O-Connor_eelgrass/Data/macro_18S/macro_18S_COVERAGE_RAREF_2000.rds")
macro18S_COVERAGE_RAREF_2000 <- readRDS("/Users/parfreylab/Desktop/lab_member_files/bia/Calvert_O-Connor_eelgrass/Data/macro_18S/macro_18S_COVERAGE_RAREF_2000.rds")

### Average otu tables from all iterations to get a final robust table ###
subset_phylo_objects_2000 <- macro18S_COVERAGE_RAREF_2000[c(1:2000)]
...
otu_tables_2000 <- lapply(subset_phylo_objects_2000, function(z) as.data.frame(t(phyloseq::otu_table(z))))

# average all matrices to get the mean abundance across all iterations

average_otu_tables_2000 <- Reduce("+",otu_tables_2000)/length(otu_tables_2000)
# IMPORTANT! NEED TO ROUND IT OTHERWISE iNEXT WILL NOT WORK!
average_otu_tables_2000_round <- average_otu_tables_2000 %>% mutate_at(vars(starts_with("ASV")), funs(round(., 0)))
# add SampleID column back
average_otu_tables_2000_round$SampleID <- rownames(average_otu_tables_2000) 
average_otu_tables_2000_round <- average_otu_tables_2000_round %>% 
  select(SampleID, everything())

write.csv(average_otu_tables_2000_round, "/Users/parfreylab/Desktop/lab_member_files/bia/Calvert_O-Connor_eelgrass/Data/macro_18S/macro_18S_average_otu_tables_2000.csv", quote=F, row.names=F )
