...
macro18S_COVERAGE_RAREF_1000_COR_SING <- phyloseq_coverage_raref(physeq=macro18S_final_for_CB, coverage = 0.9, iter = 1000, replace = F, correct_singletons = F, drop_lowcoverage = T)

saveRDS(macro18S_COVERAGE_RAREF_1000_COR_SING, "/Users/parfreylab/Desktop/lab_member_files/bia/Calvert_O-Connor_eelgrass/Data/macro_18S/macro_18S_COVERAGE_RAREF_1000_COR_SING.rds")
macro18S_COVERAGE_RAREF_1000_COR_SING <- readRDS("/Users/parfreylab/Desktop/lab_member_files/bia/Calvert_O-Connor_eelgrass/Data/macro_18S/macro_18S_COVERAGE_RAREF_1000_COR_SING.rds")

### Average otu tables from all iterations to get a final robust table ###
subset_phylo_objects_1000_COR_SING <- macro18S_COVERAGE_RAREF_1000_COR_SING[c(1:1000)]
...
otu_tables_1000_COR_SING <- lapply(subset_phylo_objects_1000_COR_SING, function(z) as.data.frame(t(phyloseq::otu_table(z))))

# average all matrices to get the mean abundance across all iterations

average_otu_tables_1000_COR_SING <- Reduce("+",otu_tables_1000_COR_SING)/length(otu_tables_1000_COR_SING)
# IMPORTANT! NEED TO ROUND IT OTHERWISE iNEXT WILL NOT WORK!
average_otu_tables_1000_COR_SING_round <- average_otu_tables_1000_COR_SING %>% mutate_at(vars(starts_with("ASV")), funs(round(., 0)))
# add SampleID column back
average_otu_tables_1000_COR_SING_round$SampleID <- rownames(average_otu_tables_1000_COR_SING) 
average_otu_tables_1000_COR_SING_round <- average_otu_tables_1000_COR_SING_round %>% 
  select(SampleID, everything())

write.csv(average_otu_tables_1000_COR_SING_round, "/Users/parfreylab/Desktop/lab_member_files/bia/Calvert_O-Connor_eelgrass/Data/macro_18S/macro_18S_average_otu_tables_1000_COR_SING.csv", quote=F, row.names=F )
