...
library(tidyverse)
library(reshape2)
library(readr)#MW: RS added
library(taxize)#MW: RS added
library(dplyr)#MW: RS added
library(ggplot2)#MW: RS added
library(tidyr)#MW: RS added
...
# read data
#d14 <- read.csv( "../Data/Grazers/hakai_grazers_2014.csv", stringsAsFactors = FALSE ) #MW
d14 <- read.csv( "~/R_RDS/GRAZERS/Data/Grazers/hakai_grazers_2014.csv", stringsAsFactors = FALSE )
...
# read data
#d15 <- read.csv( "../Data/Grazers/hakai_grazers_2015.csv", stringsAsFactors = FALSE ) #MW
d15 <- read.csv( "~/R_RDS/GRAZERS/Data/Grazers/hakai_grazers_2015.csv", stringsAsFactors = FALSE, check.names = F) #MW, added check.names, because it was throwing up an error that column names weren't unique, also there is no date
...
# rename and organize columns
l15 <- l15 %>%
  select( year, date=Date, site=Site, sample=Sample, sieve='Sieve size (mm)', taxon, count )#MW I changed this sieve size column name, it didn't get all funky because I used the check.names = F above
#### END OF 2015
...
# read data
#d16 <- read.csv( "../Data/Grazers/hakai_grazers_2016.csv", stringsAsFactors = FALSE ) #MW
d16 <- read.csv( "~/R_RDS/GRAZERS/Data/Grazers/hakai_grazers_2016.csv", stringsAsFactors = FALSE)
...
# read data
#d17 <- read.csv( "../Data/Grazers/hakai_grazers_2017.csv", stringsAsFactors = FALSE ) #MW
d17 <- read.csv( "~/R_RDS/GRAZERS/Data/Grazers/hakai_grazers_2017.csv", stringsAsFactors = FALSE)
# data is already in long format
...
#rename columns
#MW below I have added what I called the sites for the seagrass-microbe paper
#new_site_indval <- rename(new_site_indval, c("rowname"="OTU",
                                             #"new.choked_sandspit"="new.Choked_1",
                                             #"new.choked_south_pigu"="new.Choked_2",
                                             #"new.triquet_north"="new.Triquet_1", 
                                             #"new.triquet_bay_south"="new.Triquet_2", 
                                             #"new.goose_southeast"="new.Goose_1", 
                                             #"new.goose_southwest"="new.Goose_2", 
                                             #"new.mcmullin_north"="new.McMullin_1", 
                                             #"new.mcmullin_south"="new.McMullin_2")) 
...
f14$site[ f14$site=="Lower Choked" ] <- "inner choked" #MW I call this Choked_1
...
f14$site[ f14$site=="Triquet/No Name Cove" ] <- "triquet south" #MW: this is triquet bay
...
#MW: filter out the most consistently sampled sites for initial interannual analysis 
m_filt <- filter(m, site == c('inner choked', 'sandspit', 'triquet north', 'triquet south'))

#MW: filter out 2015 for best spatial analysis
m_spat <- filter(m, year == 2015)
...
## Quality check and modified taxa names in excel, reload master species list into R: #MW, RS added below here
mq <- read.csv( "~/R_RDS/GRAZERS/R Code and Analysis/output data/unique_taxa_edited_20190423_RS.csv")
mq_df <- as.data.frame(mq)
mq_df$remove <- as.integer(mq_df$remove)
mq_removed <- dplyr::filter(mq, remove == 1)
mq_keep <- mq_df %>% filter(is.na(remove))
unitax_qc <- sort(unique(mq_keep$Modified))

## Fill in taxa information using taxize package

#name_query = tnrs(unitax_qc, use.names=FALSE, source = "iPlant_TNRS")
unitax_class <- classification(unitax_qc, db = 'itis')

#turn into an exportable dataframe 
unitax_class_df <- tibble(names = names(unitax_class), unitax_class) %>% 
  unnest() %>% 
...
write.csv(unitax_class_df, "R Code and Analysis/output data/unique_class_df.csv", row.names = TRUE )
