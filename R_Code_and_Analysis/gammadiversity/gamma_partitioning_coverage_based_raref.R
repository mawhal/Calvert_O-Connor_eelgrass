...
#############################################
############ Macroeuk18S ASV level ##########
#############################################
### Read table metadata and abundances
macroeuk18S_ASV <- read.csv("Data/R_Code_for_Data_Prep/master_data/MASTER_macroeuk18S_ASV_level_1000_COR_SING_COVERAGE_RAREF.csv", header=T)
names(macroeuk18S_ASV)[1:13]

# split by year
comm.years <- split( macroeuk18S_ASV, macroeuk18S_ASV$year )
lapply(comm.years, dim)
...
add.plot2 <- add.plot %>% 
  gather( value="value", key = "key", - year, -level)
write.csv(add.plot2, file="R_Code_and_Analysis/output_data/adipart_macroeuk18S_ASV_level.csv", row.names=F)
...
g1 + geom_bar(data=add.plot2 %>% dplyr::filter(key == "null"),
              aes(x=year + 0.15, fill=level),
              stat="identity", position="stack", width=0.3, alpha=0.5)
ggsave( "R_Code_and_Analysis/gammadiversity/macroeuk18S_ASV_gamma_partition_additive_richness_prop.png", width=4, height=3 )
